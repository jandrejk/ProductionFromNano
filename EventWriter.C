...
void EventWriter::fillStitchingWeight(HTTEvent::sampleTypeEnum sampleType)
{
    ...
    {
        if(NUP == 0) stitchedWeight = 0.92600048;
        if(NUP == 1) stitchedWeight = 0.13544050;
        if(NUP == 2) stitchedWeight = 0.10616984;
        if(NUP == 3) stitchedWeight = 0.06370226;
        if(NUP == 4) stitchedWeight = 0.05100958;
...
void EventWriter::initTree(TTree *t, vector< pair< string, pair<string,bool> > > jecShifts_,  bool isMC_, bool isSync_, vector<string> metShifts_ ){
    ...
    TFile wsp("utils/CorrectionWorkspaces/htt_scalefactors_sm_moriond_v2.root");
